...
def main_idlv(fout, samples, resolution, k, w=0.01, m=1.0, normalization="max_global", max_features=None, by_instance=False):
    ...
    for word in dict_words_arrayindex:
        ...
        for i in range(last_index+1):
            ...
            if freq_index != 0: ## if word j do not apper in index i, there is nothing to add.
                exponent = math.pow((i/(k-1)),m)
                exponent_w = math.pow(w, exponent)
                sum += (exponent_w * ( freq_index/ max_freq))

                logging.debug("index=%d; freq_index=%d; max_freq=%d; exponent=%f; exponent_w=%f; sum=%f"%(i, freq_index, max_freq, exponent, exponent_w, sum))
...
def main(dir_in, dir_out, resolution, k, w, m):
    ...
    for f in files:
        idlv_file = 'IDLV_list_'+get_file_name(f)+'_'+str(resolution)+'r'+'_st.idl'
        idlv_file = os.path.join(dir_out, idlv_file)
        samples = open(f,'r')

        main_idlv(idlv_file, samples, resolution, k, w, m)  ## remember w, and m are optional in main_idlv
